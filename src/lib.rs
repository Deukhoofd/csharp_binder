        ...
        return match path.segments.last() {
            Some(v) => {
                match v.ident.to_string().as_str() {
                    ...
                    "bool" => Err(UnsupportedError("Found a boolean type. Due to differing sizes on different operating systems this is not supported for extern C functions.".to_string())),
                    "str" => Err(UnsupportedError("Found a str type. This is not supported, please use a char pointer instead.".to_string())),
